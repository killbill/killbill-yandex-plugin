...
helpers do
  def plugin(session = {})
    ::Killbill::Yandex::PrivatePaymentPlugin.new(:yandex,
...
# curl -v http://127.0.0.1:9292/plugins/killbill-yandex/form
get '/plugins/killbill-yandex/form', :provides => 'html' do
  ...
  @form = plugin(session).payment_form_for(order_id, account_id, :yandex, options) do |service|
    # Add your custom hidden tags here, e.g.
    service.scid = config[:yandex][:scid]
    submit_tag 'Submit'
  end
